...
from offset_detection import offset_detection_on_spectrograms, offset_detection_based_neg_slope_energy  
...
audio= 'C:\\Users\\anton\\Data_normalised\\Testing_set\\chick367_d1.wav'
txt_file= 'C:\\Users\\anton\\Data_normalised\\Testing_set\\chick367_d1.txt'
...
save_evaluation_results_path = r'C:\\Users\\anton\\Chicks_Onset_Detection_project\\Offsets_testing_collar_time'
...
n_events_list = []
file = glob.glob(os.path.join(audio, "*.wav"))
...
gt_onsets = my_eval.get_reference_onsets(txt_file)
gt_offsets = my_eval.get_reference_offsets(txt_file)

    # offsets_per_file = offset_detection_on_spectrograms(file, gt_onsets)
offsets_per_file = offset_detection_based_neg_slope_energy(audio, gt_onsets, gt_offsets)


chick = os.path.basename(audio)[:-4]
chick= file
...
Fscore, precision, recall, TP, FP, FN = f_measure(gt_offsets, offsets_per_file , window= EVAL_WINDOW)
