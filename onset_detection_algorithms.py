...
def thresholded_phase_deviation(file_name, hop_length=441, sr=44100, spec_num_bands=64, spec_fmin=1800, spec_fmax=6000, 
    ...
    if len(pp_threshold) == 1:
        peaks = madmom.features.onsets.peak_picking(phase_ons_fn, threshold=pp_threshold[0], smooth=None, pre_avg=pp_pre_avg, post_avg=pp_post_avg, pre_max=pp_pre_max, post_max=pp_post_max)
        tpd_onsets_seconds= np.array([(peak * hop_length / sr ) for peak in peaks])
        return tpd_onsets_seconds
    else:
        tpd_onsets_seconds = []
        for threshold in pp_threshold:
            peaks = madmom.features.onsets.peak_picking(phase_ons_fn, threshold=threshold, smooth=None, pre_avg=pp_pre_avg, post_avg=pp_post_avg, pre_max=pp_pre_max, post_max=pp_post_max))
            tpd_onsets_seconds.append(np.array([(peak * hop_length / sr ) for peak in peaks]))
        return tpd_onsets_seconds, 
