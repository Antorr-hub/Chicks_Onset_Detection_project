...
# High frequency content grid search:

print('********************* Starting grid search on High frequency content')
output_file = os.path.join(output_directory, "HFC.json")
#non-changing parameters
hop_length= 441
sr= 44100

# Ranges for Grid search paramseters for hfc algorithm
num_bands_range =[15, 24, 32]
fmin_range=  [ 1800, 2000]
fmax_range = [5000, 6000]
fref_range = [2500, 2800]

threshold_range = [1.8, 2.5, 3]
pre_avg_range =[0 1, 3 ]
...
parameter_combinations = list(product(num_bands_range, fmin_range, fmax_range, fref_range, threshold_range, pre_avg_range, post_avg_range, pre_max_range, post_max_range, window_range, global_shift_range))

overall_fmeasure_and_parameters = {}
for i in tqdm(range(len(parameter_combinations))):
    num_bands, fmin, fmax, fref, threshold, pre_avg, post_avg, pre_max, post_max, eval_window, correction_shift = parameter_combinations[i]
    list_fscores_in_set = []
    ...
    overall_fmeasure_and_parameters[overall_fmeasure_in_set] = {'num_bands': num_bands, 'fmin': fmin, 'fmax': fmax, 'fref': fref, 'threshold': threshold, 'pre_avg': pre_avg, 'post_avg': post_avg, 'pre_max': pre_max, 'post_max': post_max, 'window': eval_window, 'correction_shift': correction_shift}

# Save the results in a JSON file
with open(os.path.join(output_directory, 'HFC_gridsearch_results.json'), "w") as file:
