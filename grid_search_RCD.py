...
which_search =  'evaluation' #, 'peak_picking', 'evaluation'


if which_search == 'peak_picking':
    output_file = os.path.join(output_directory, "RCD_search_peak_picking.json")
elif which_search == 'evaluation':
    output_file = os.path.join(output_directory, "RCD_search_evaluation.json")
...
# No grid search for input features for RCD algorithm 
    
# peak picking parameters
threshold_range = [30, 50, 70]
threshold = 70
...
for i in tqdm(range(len(parameter_combinations))):
    ...
    for filepath in list_files:
        ...
        RCD_predictions_in_seconds, RCD_activation_frames = onset_detectors.rectified_complex_domain(filepath, hop_length=hop_length,sr=sr , pp_threshold=threshold, 
                                                                                                     pp_pre_avg=pre_avg, pp_post_avg=post_avg,pp_pre_max=pre_max, 
                                                                                                     pp_post_max=post_max, visualise_activation=True)
        ...
        gt_onsets, RCD_predictions_in_seconds, RCD_activation_frames = eval.discard_events_outside_experiment_window(exp_start,exp_end, 
        ...
        RCD_predictions_in_seconds = eval.global_shift_correction(RCD_predictions_in_seconds, correction_shift )
        
        fmeasure, precision, recall, _, _, _  = f_measure(gt_onsets, RCD_predictions_in_seconds, window=eval_window)
    ...
    overall_fmeasure_and_parameters[i] = {'f_measure':overall_fmeasure_in_set, 'threshold': threshold, 'pre_avg': pre_avg, 'post_avg': post_avg, 'pre_max': pre_max, 'post_max': post_max, 'window': eval_window, 'correction_shift': correction_shift}
