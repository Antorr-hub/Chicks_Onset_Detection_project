...
n_events_list = []
for file in tqdm.tqdm(range(len(list_files))):

    # for alg in list_algorithms:

    hfc_pred_scnd = onset_detectors.high_frequency_content(file)
    # save prediction to file
    hfc_predictions_seconds_df = pd.DataFrame(hfc_pred_scnd, columns=['onset_seconds'])
    hfc_predictions_seconds_df.to_csv(os.path.join(save_predictions_path, file[:,-4] +'_HFCpredictions.csv'), index=False)
    # ##evaluate
    # get ground truth onsets
    gt_onsets = eval.get_reference_onsets(file.replace('.wav', '.txt'))
    individual_scores = mir_eval.onset.evaluate(gt_onsets, hfc_pred_scnd, window=0.05)
